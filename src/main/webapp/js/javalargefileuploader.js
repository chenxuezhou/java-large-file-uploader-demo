...
function JavaLargeFileUploader() {
	...
	progressPollerRefreshRate = 1000;//进度刷新时间
	maxNumberOfConcurrentUploads = 5; //向后台发起最多上传个数
	autoRetry = true; //是否重试
	autoRetryDelay = 5000; //每次重试隔几秒
	errorMessages = new Object();
	errorMessages[0] = "请求失败未知错误，请联系管理员";
	...
	exceptionsRetryable = [0,3,7,8,10,11,15,16];//异常重试表
    ...
    /**
	 * 
     * @param referenceToFileElement  事件源[所有上传文件]
     * @param startCallback
	...
	this.fileUploadProcess = function (referenceToFileElement, startCallback, progressCallback,
		...
		for (fileKey in allFiles) {
			...
			//pendingFiles全局对象，存着当前被上传的文件，[刚开始是空的]
